...
class httpHandler(http.server.SimpleHTTPRequestHandler):
   ...
   def _set_cookie(self):
      if self._get_cookie():
         print("a")
         self.cookie["_session"] = self._get_cookie()
      else:
         self.cookie["_session"] = str(time.time())
         print("yeah")
   ...
   def cart(self,form):
      redisHost = "cluster.ofrdjf.clustercfg.use1.cache.amazonaws.com"
      redisPort = 6379 
      c = redis.Redis(host=redisHost, port=redisPort, decode_responses=True)
      redis.Redis()
      ...
      for key in keys:
         val = form[key].value
         c.hset(cookie,key,val)
         x = c.hget(cookie,key)
         print(x)
      y = c.hget(cookie,keys[0])
      print(y)
   ...
   def upload(self, form):
      ...
      s3.upload_file(UPLOAD_FILE, S3_BUCKET_NAME, 'file.uploaded')
